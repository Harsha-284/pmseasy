...
function intialRoomLoad(
  ...
  let checkinDateBooking = document.querySelector(
    'input[name="checkinDateBooking"]'
  ).value;
  let checkoutDateBooking = document.querySelector(
    'input[name="checkoutDateBooking"]'
  ).value;
  let old_childBooking = parseInt(
    document.querySelector('input[name="childBooking"]').value
  );
  let old_adultBooking = parseInt(
    document.querySelector('input[name="adultBooking"]').value
  );
  let no_of_rooms = parseInt(
    document.querySelector('input[name="noOfRooms"]').value
  );
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      selectedRooms.set(roomtypeid, {
        ...
        adultBooking,
        no_of_rooms,
        extraCharge: parseInt(data.extra_charge),
        old_adult: old_adultBooking,
        old_child: old_childBooking,
      });
      document.getElementById(
        `1stmodalAvailibility-${roomtypeid}`
...
function search_bookings(
  ...
  let old_childBooking = document.querySelector(
    'input[name="childBooking"]'
  ).value;
  let old_adultBooking = document.querySelector(
    'input[name="adultBooking"]'
  ).value;
...
function renderCards() {
  ...
  roomsToDisplay.forEach(async (value, key) => {
    ...
    fetchRatePlan(value.roomtypeid)
      .then((rateplan) => {
        document.getElementById(`rateplan-${value.roomtypeid}`).innerHTML =
          rateplan.str;
        document.getElementById(
          `room-occupancy-${value.roomtypeid}`
        ).innerHTML =
          rateplan.occupancy !== undefined
            ? rateplan.occupancy
            : "Rates not defined";
      })
      .catch((err) => {
      ...
      if (data && data.data && Array.isArray(data.data)) {
        ...
        data.data.forEach((item) => {
          ...
          const textNode = document.createTextNode(
            `${item.room_rate_plan} ₹${item.fulldaytariff}`
          );
...
async function getFulldaytariffRatePlan(rateplanid, validfrom, validto) {
  try {
    const response = await fetch(
      `bookingajax.php?rateplanids=${rateplanid}&validfrom=${validfrom}&validto=${validto}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
async function fetchRoomRates(
  ...
  let response = await getFulldaytariffRatePlan(
    room_rate_plan,
    checkinDateBooking,
    checkoutDateBooking
  );
...
async function bookRoom(hotelcode) {
  ...
  let date_of_payment = document.querySelector(
  ...
  let actual_payment = document.querySelector(
    'input[name="fixed-rate-input"]'
  ).value;
  let discount_type =
    mode_of_payment === "discount"
      ? document.getElementById("discountChoiceDropdown").value
      : "";
  let flat_discount = "";
  ...
  if (
    !roomData ||
    !roomData.checkinDateBooking ||
    !roomData.checkoutDateBooking
  ) {
    showMessageBoxuserdetails(
  ...
  if (!mode_of_payment) {
    showMessageBoxuserdetails("Please select mode of payment.", "warning");
    return;
  }

  if (
    mode_of_payment === "neft/rtgs/imps" ||
    mode_of_payment === "upi" ||
    ...
    if (!txn_id || !date_of_payment || !amount_inp) {
      showMessageBoxuserdetails(
        "Please fill in all transaction details for the selected payment mode.",
        "warning"
      );
      return;
    }
    ...
    if (!date_of_payment || !amount_inp) {
      showMessageBoxuserdetails(
        "Please provide the payment date and amount for cash payments.",
        "warning"
      );
      return;
    }
    ...
    if (
      ...
      showMessageBoxuserdetails(
        "Please provide all cheque details for cheque payments.",
        "warning"
      );
      return;
    }
    ...
    if (!discount_type || !discount_value || !date_of_payment) {
      showMessageBoxuserdetails(
        "Please provide the discount details.",
        "warning"
      );
      return;
    }
  ...
  let alltariff = await fetchRoomRates(
    no_of_rooms,
    checkinDateBooking,
    checkoutDateBooking
  );
