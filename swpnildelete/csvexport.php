...
/*$sql = "select
vr.hotel as HotelName,
vr.roomtype as RoomName,
'???' as CancellationDescription,
coalesce(p.path,'') as RoomImage,
vr.id as RoomCode,
'???' as MaxAccomodation,
vr.adults as MaxAdults,
vr.children as MaxChildren,
vr.fulldaytariff as RoomPricePerDay,
'???' as Discount,
'???' as RoomTotalPrice,
vr.hotelid as RoomAmenityid,
vr.hotelid as Hotelid
from view_roomtypes vr left join pictures p on (p.type='Room' and vr.id=p.propertyid and coalesce(p.approved,0)=1) where vr.hotel_approved=1";
*/

$sql = "select  
h.id as HotelId,
u.company as HotelName,
...
h.minidescription as HotelOverview,
h.fulldescription as AboutHotel,
h.latitude as Latitude,
h.longitude as Longitude,
'' as HotelPolicy
from users u left join hotels h on h.user=u.id left join locations l on l.id=h.location left join cities c on c.id=l.city left join states s on s.id=c.state left join countries con on con.id=s.country where u.approved=1";		 	 					


// $sql = "SELECT `hotelid`,`hotel`,path FROM `view_roomtypes` WHERE 1 ";
...
//echo $sql; SELECT u.id,u.company,p.path from users u, pictures p where (u.id = p.propertyid) and p.title = 'Hotel Profile Picture' 
...
while ($row = $result->fetch_assoc())
{
	for ($i = 0; $i < $columns_total; $i++)
	{
		...
		{
			if($row[$colnames[$i]] != "")
				$output .= '"'.base_url.'frotels/images/propertypics/hotel/medium/'.$row[$colnames[$i]].'",';
			else
				$output .= ',';
		}
		else
