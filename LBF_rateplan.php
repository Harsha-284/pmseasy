...
if (!isset($_SESSION['groupid'])) { ?>
	...
	if (isset($_POST['oldroomnumbers'])) {
		...
		foreach ($_POST['oldroomnumbersid'] as $key => $oldroomnumberid) {
			...
			if (content_access("room_rate_plan", $oldroomnumberid))
				...
				$conn->query("update room_rate_plans set room_rate_plan='$oldroomnumber',deleted=0 where id=$oldroomnumberid");
		}
	...
	if (isset($_POST['newroomnumbers'])) {
		...
		foreach ($_POST['newroomnumbers'] as $newroomrate) {

			if ($newroomrate != "")

				insert("insert into room_rate_plans (roomtype,room_rate_plan,deleted) values ($id,'" . nosql($newroomrate) . "',0)");
		}

		$msg = "Room Rates are set";
	...
	<body style="padding-top:0px;">
		...
		<div class="the-box" style="padding-bottom:0px;">
			...
			<h4 class="small-text">SET ROOM RATE PLAN '<?= strtoupper($row['roomtype']) ?>'</h4>
			...
			<div class="row">
				...
				<form method="post" action="?id=<?= $id ?>" id="rnform">
					...
					<?php $result = $conn->query("select id,room_rate_plan from room_rate_plans where roomtype=$id and deleted=0");
							...
							<div id="rn<?= $i ?>">
								...
								<div class="col-xs-10">
									...
									<div class="form-group">
										...
										<input type="text" name="oldroomnumbers[]" class="form-control" placeholder="E.g Deluxe (with dinner)" value="<?= $row['room_rate_plan'] ?>" required>
					...
					<div class="col-xs-12">
						...
						<div class="form-group">
							...
							<a href="javascript:void(0)" onClick="addroomnumber()">+Add more Rates</a>
