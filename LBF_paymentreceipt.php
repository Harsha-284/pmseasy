...
<body>
  ...
  $payment = execute("select id,payment_type, date_of_payment, txnid, cheque_bank, cheque_no, cheque_date, comment,amount from payment_mode where bookingid=$row[id] and id=$_GET[paymentid]");

  $payment_receipt=execute("select id from payment_mode_receipt where paymentmodeid=$payment[id] and bookingid=$row[id]");

  $checkindate = new DateTime($row['checkindatetime']);
  $checkoutdate = new DateTime($row['checkoutdatetime']);
  $paymentdate = new DateTime($payment['date_of_payment']);
  ...
  <div class="wrapper" id="wrapper">
    ...
    <div class="innerwrapper">
      <div class="mid-section">
        <div class="receipt">
          <p class="receipt-no font-bold calibri text-tup">
            Receipt no&nbsp;<?=$payment_receipt['id']?>
          </p>
        </div>
        ...
        <div class="date calibri font-bold">
          ...
          <input
            type="text"
            class="date-section font-bold font-size-22px calibri"
            value="<?= $paymentdate->format("d-m-Y") ?>"
            readonly="" />
        </div>
      </div>
      <div class="content divnm">
        <div class="first-line calibri">
          <p class="receipt-no">
            ...
            <input
              class="client-name font-bold calibri font-size-22px margin-top"
              type="text"
              value="<?= $row['fullname'] ?>"
              readonly="" /><span class="colon-d">:</span>
          </p>
          <p class="receipt-no2">Room details</p>
          <div class="overflow-hidden check-dn">
            <p class="receipt-no float-left width-38">
              Date<input
                class="rupee font-bold calibri font-size-22px oneline width-38 margin-top float-left width-"
                type="text"
                value="<?= $checkindate->format("d-m-Y") ?>"
                readonly="" /><span class="colon-d">:</span>
            </p>
          </div>
          ...
          <div class="overflow-hidden check-dn">
            <p
              class="receipt-no float-left width-38"
              ...
              Room Type<input
                ...
                style="
                    ...
                    overflow: hidden;
                  " /><span class="colon-d" style="margin-right: 10px">:</span>
            </p>
          </div>

          <p class="receipt-no">
            Mode of Payment<input
              class="rupee font-bold calibri font-size-22px oneline width-58 margin-top margin-left-138"
              type="text"
          ...
          <p class="receipt-no">
            The sum of rupees
            <input
              class="rupee font-bold calibri font-size-22px oneline margin-top"
              type="text"
              name="number-to-word"
              value=""
              readonly="" /><span class="colon-d">:</span>
          ...
          <div class="width-100 float-left">
            <p class="receipt-no vmc40">
              Note <span class="colon-d">: </span>
            </p>
            <div
              class="rupee font-bold calibri font-size-22px oneline margin-left margin-top mvp60"
              style="height: 28px"> <?=$payment['comment']?></div>
            <p></p>
          </div>
        </div>
      </div>
      <div class="last-content" style="width: 100%">
        <div class="box-size">
          <div class="rupee-box all-center">
            ...
            <p class="amount calibri font-bold font-size-22px totalamt">
              ₹ <?= $_GET['amount'] ?>
            </p>
          </div>
          <div class="claim"></div>
  ...
  <div id="bttns" style="width: 1034px; margin: auto; margin-top: 30px"></div>
  <script>
    let word= numberToWords('<?=$_GET['amount']?>')
