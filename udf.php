...
$google_playstore_link	= "https://play.google.com/store/apps/details?id=com.aws.frotels&hl=en";
$apple_playstore_link	= "https://play.google.com/store/apps/details?id=com.aws.frotels&hl=en";
...
/***************************** //System Variables *******************************/
include_once("PHPMailer/mymailer.php");
...
class pagination
{
	...
	var $total_pages_;
	var $row_count_;
	var $current_page_;  // Declare current_page_
	var $result_;
	var $row_;
...
class pagination1
{
	...
	function __construct($q0001, $c0001 = 1, $page_size_ = 10)
	{
		global $conn;
		// Run count query to get total rows
		$this->result_ = $conn->query("select count(*) as cnt from ($q0001) as bq");
		$this->row_count_ = $this->result_->fetch_assoc()['cnt'];  // Assign row count

		// Calculate total pages
		$this->total_pages_ = ceil($this->row_count_ / $page_size_);

		// Set current page, ensuring it's within valid range
		$this->current_page_ = ($c0001 && $c0001 <= $this->total_pages_) ? $c0001 : 1;

		// Calculate offset
		$this->offset_ = ($this->current_page_ - 1) * $page_size_;
	}

	// Create a function to expose current_page_
...
/***************************************** GMAIL PHPMAILER 5.2.0 (Now deprecated) *************************************************/
if(0)
{
...
/***************************************** ZOHO PHPMAILER 5.2.0 (Now deprecated) ************************************************/

if(0)
{
	require_once (mailer_path);
	function myemail3($to,$subject,$data,$cc,$bcc,$from,$attachment="")
	{
		$mail = new PHPMailer();
...
{
	// include("PHPMailer/mymailer.php");
}
/**************************************************************************************************************/
...
function toastrxxxxxx($title,$message,$color="success")
{?>
	<div id="toastTypeGroup" style="display:none">
		<input id="title" type="text" class="form-control" placeholder="Enter a title ..." value="<?=$title?>" />
		<textarea class="form-control" id="message" rows="3" placeholder="Enter a message ..."><?=$message?></textarea>
		<input id="closeButton" type="checkbox" value="checked" checked />Close Button
		...
		<input type="radio" name="toasts" value="success" <?=$color=="success"?"checked":""?> />Success
		<input type="radio" name="toasts" value="info" <?=$color=="info"?"checked":""?> />Info
		<input type="radio" name="toasts" value="warning" <?=$color=="warning"?"checked":""?> />Warning
		<input type="radio" name="toasts" value="error" <?=($color=="error" or $color=="danger")?"checked":""?> />Error
...
function savefile($filefieldname, $fpath, $width = 0, $key56 = -1)
{
    // Handle multi-file or single-file uploads
    $file = ($key56 == -1) ? $_FILES[$filefieldname] : $_FILES[$filefieldname][$key56];
    $fileTmpName = ($key56 == -1) ? $_FILES[$filefieldname]['tmp_name'] : $_FILES[$filefieldname]['tmp_name'][$key56];
    $fileName = ($key56 == -1) ? $_FILES[$filefieldname]['name'] : $_FILES[$filefieldname]['name'][$key56];
    ...
    $newfilename = str_replace(" ", "", rand() . basename($fileName));

    // Check if the file is an image
    if (in_array($file_extension, $allowed_image_extensions)) {
        $source_properties = getimagesize($fileTmpName);
        $image_type = $source_properties[2];
        $asp_ratio = $source_properties[0] / $source_properties[1];

        // Image handling and resizing (same as before)
        if ($width > 0) {
            // Resize based on width
            if ($image_type == IMAGETYPE_JPEG) {
                $image_resource_id = imagecreatefromjpeg($fileTmpName);
                $target_layer = fn_resize($image_resource_id, $source_properties[0], $source_properties[1], $width, ($width / $asp_ratio));
                imagejpeg($target_layer, $fpath . $newfilename);
            } elseif ($image_type == IMAGETYPE_GIF) {
                $image_resource_id = imagecreatefromgif($fileTmpName);
                $target_layer = fn_resize($image_resource_id, $source_properties[0], $source_properties[1], $width, ($width / $asp_ratio));
                imagejpeg($target_layer, $fpath . $newfilename);
            } elseif ($image_type == IMAGETYPE_PNG) {
                smart_resize_image($fileTmpName, $width, 0, true, $fpath . $newfilename);
            }
        } else {
            // Save original file in different sizes
            if ($image_type == IMAGETYPE_JPEG) {
                // Save in different sizes for JPEG
                $image_resource_id = imagecreatefromjpeg($fileTmpName);
                $target_layer = fn_resize($image_resource_id, $source_properties[0], $source_properties[1], 1200, (1200 / $asp_ratio));
                imagejpeg($target_layer, $fpath . "big/" . $newfilename);
                $target_layer = fn_resize($image_resource_id, $source_properties[0], $source_properties[1], 400, (400 / $asp_ratio));
                ...
                // Save in different sizes for GIF
                $image_resource_id = imagecreatefromgif($fileTmpName);
                $target_layer = fn_resize($image_resource_id, $source_properties[0], $source_properties[1], 1200, (1200 / $asp_ratio));
                imagejpeg($target_layer, $fpath . "big/" . $newfilename);
                $target_layer = fn_resize($image_resource_id, $source_properties[0], $source_properties[1], 400, (400 / $asp_ratio));
                ...
                smart_resize_image($fileTmpName, 1200, 0, true, $fpath . "big/" . $newfilename);
                smart_resize_image($fileTmpName, 400, 0, true, $fpath . "medium/" . $newfilename);
                smart_resize_image($fileTmpName, 100, 0, true, $fpath . "small/" . $newfilename);
            }
        }
        return $newfilename;
    } 
    ...
    elseif (in_array($file_extension, $allowed_document_extensions)) {
        // Move file to destination directory for documents
        if (move_uploaded_file($fileTmpName, $fpath . $newfilename)) {
            return $newfilename;
        } else {
...
function decode_permissions($data1){
	$decoded_data = html_entity_decode($data1);
...
function add_hotel_task_log($user_id,$hotel_id,$msg,$module){
	insert("insert into hotel_task_log (user_id,hotel_id,message,module) values ('$user_id','$hotel_id','$msg','$module')");
}
...
function content_access($tablename94,$id37,$proptype="Hotel")
{
	...
	if($tablename94 == "room_rate_plan")
	{
		$res_row = execute("select count(*)cnt from room_rate_plans rp join roomtypes r on rp.roomtype=r.id join hotels h on r.hotel=h.id where h.id=$_SESSION[hotel] and rp.id=$id37");
		if($res_row['cnt'] == 1)
			return true;
		else
			return false;
...
function limittext($a35,$lim=20)
{
	$str2365 = substr($a35, 0, $lim);
	if(strlen($a35)>$lim)
...
if(isset($_SESSION['groupid']))
{
	...
	if($_SESSION['groupid']==0 or $_SESSION['groupid']==1)
	{
		if($_SESSION['groupid']==0)
		{
			...
			$result = $conn->query("select h.id,u.company from hotels h left join users u on u.id=h.user left join locations l on h.location=l.id LEFT JOIN (
        SELECT ps1.*
    ...
    ) ps ON ps.hotel_id = h.id where u.city=$_SESSION[city] AND ps.start_date <= CURDATE()
        AND ps.end_date >= CURDATE() order by u.company");
			if($result->num_rows > 0)
			{
				while($row=$result->fetch_assoc())
					$current_hotel .= "<option value='".$row['id']."' ".selected($row['id'],col("current_hotel",$_SESSION['hotel'])).">".$row['company']." (".$row['id'].")</option>";
			}
		...
		else if(($_SESSION['groupid']==1 or $_SESSION['groupid']==0) and col("Pg")=="myhotels")
		{
			if($_SESSION['groupid']==1)
			{
				...
				$result = $conn->query("select h.id,u.company from hotels h join users u on u.id=h.user LEFT JOIN (
        SELECT ps1.*
        FROM pms_subscriptions ps1
        JOIN (
            SELECT hotel_id, MAX(id) AS max_id
    ...
    ) ps ON ps.hotel_id = h.id where h.admin=$_SESSION[id] AND ps.start_date <= CURDATE()
        AND ps.end_date >= CURDATE()");
				while($row=$result->fetch_assoc())
				{
					$current_hotel .= "<option value='".$row['id']."' ".selected($row['id'],$_SESSION['hotel']).">".$row['company']." (".$row['id'].")</option>";
				}
				$current_hotel .= "</select>";
			}
			else
    ...
    ) ps ON ps.hotel_id = h.id where h.admin=$hadmin[admin] AND ps.start_date <= CURDATE()
        AND ps.end_date >= CURDATE()");
				while($row=$result->fetch_assoc())
				{
					$current_hotel .= "<option value='".$row['id']."' ".selected($row['id'],$_SESSION['hotel']).">".$row['company']." (".$row['id'].")</option>";
        ...
        AND ps.end_date >= CURDATE()");
			while($row=$result->fetch_assoc())
			{
				$current_hotel .= "<option value='".$row['id']."'>".$row['company']." (".$row['id'].")</option>";
			}
			$current_hotel .= "</select>";
		}
	}
...
if(0 && !strpos($_SERVER['PHP_SELF'],"ajaxproc.php") and !strpos($_SERVER['PHP_SELF'],"railyatri.php") and !strpos($_SERVER['PHP_SELF'],"bookingajax.php") and !strpos($_SERVER['PHP_SELF'],"update_reservation.php") and !strpos($_SERVER['PHP_SELF'],"webservice.php"))
{?>
	<script type="text/javascript">
