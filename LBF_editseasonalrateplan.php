...
// Fetch existing data
$query = $conn->query("
    SELECT v.*, rp.room_rate_plan, rp.roomtype
    FROM seasonal_rate_plans_validity v
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
 ...
 $record = $data;
	...
	$update = $conn->query("
        UPDATE seasonal_rate_plans_validity
	...
	if ($update) {
		...
		$msg = "Seasonal Room rate updated successfully.";
	...
	if ($msg == "Seasonal Room rate updated successfully.") { alertbox($clrclass, $msg); ?>

		<script type="text/javascript">
			window.parent.location = "admin.php?Pg=setseasonalroomplanrateview";
		</script>
