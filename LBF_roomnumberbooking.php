...
<body style="padding:0px; ">
	...
	<div class="main-card">
		...
		// Reset result pointer back to beginning (so loop can run normally)
		...
		?>
			...
			<div class="inner-container">
				<div class="brn-card" style="width:100%; display: flex;justify-content: space-between;align-items: center; padding:10px 25px; background: #dcdcdc; margin-bottom:10px;">
					<div>
					...
					<div style="padding:0 15px">
						<div class="border">
							...
							<div style="padding:0 10px 10px">
									...
									?>
										<div id="create-additional-services-<?= $i ?>" class="form-check" style="margin: 0px 15px 0px 0px">
											<input id="add-row-btn-<?= $i ?>" class="form-check-input" style="width: 16px; height: 12px; float: left; margin-right: 0px"
												...
												data-service-id-<?= $row['id'] ?>="<?= $rs_row['id'] ?>"
												data-service-name-<?= $row['id'] ?>="<?= $rs_row['service'] ?>"
												data-service-charge-<?= $row['id'] ?>="<?= $rs_row['charge'] ?>"
												data-service-gst-<?= $row['id'] ?>="<?= $gst ?>"
												data-service-vat-<?= $row['id'] ?>="<?= $vat ?>"
												data-service-total-<?= $row['id'] ?>="<?= $total ?>"
												onclick="toggleServiceRow(this, <?= $row['id'] ?>)">
								...
								<table id="additional-table-<?= $row['id'] ?>" class="table table-bordered br-none mg-top" style="border-collapse: collapse; width: 100%; border: none; margin: 5px 0px; display: none">
									<thead style="border-bottom: 1px solid #ddd;">
										<tr class="table-color" style="border-bottom: 1px solid #ddd;border-top: 1px solid #ddd;font-size: 15px">
											<th class="border-none" style="font-size: 11px; border: none; ">Sr#</th>
								...
								<script>
									...
									if (<?= $i ?> > 0) {
										document.getElementById('additional-table-<?= $row['id'] ?>').style.display = 'table'
									}
								</script>
								<div style="width: 100%">
									<button id="submit-btn-<?= $row['id'] ?>" class="btn btn-primary" style="visibility: hidden " onclick="addServices(<?= $row['id'] ?>)">Submit</button>
								</div>
							</div>
						</div>
					</div>
			...
			<script>
				...
				?>
					...
					function updateSrNumbers() {
						var rows = document.querySelectorAll('#room-additional-service-<?= $row['id'] ?> tr');
			...
			<script>
				...
				let serviceRowCount = document.getElementById('room-additional-service-<?= $row['id'] ?>').rows.length;
...
<script>
	function toggleServiceRow(checkbox, id) {
		var tableBody = document.getElementById('room-additional-service-' + id);
		var serviceId = checkbox.getAttribute('data-service-id-' + id);
		var serviceName = checkbox.getAttribute('data-service-name-' + id);
		var serviceCharge = checkbox.getAttribute('data-service-charge-' + id);
		var gst = checkbox.getAttribute('data-service-gst-' + id);
		var vat = checkbox.getAttribute('data-service-vat-' + id);
		var total = checkbox.getAttribute('data-service-total-' + id);

		console.log('clicked', serviceId, serviceName, serviceCharge, gst, vat, total);

		if (checkbox.checked) {
			// Add row
			document.getElementById('additional-table-' + id).style.display = 'table'
		...
		document.getElementById('submit-btn-' + id).style.visibility = selectedServices > 0 ? 'visible' : 'hidden';
